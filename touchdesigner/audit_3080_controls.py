"""Exercise every Recovered Homes control in the active RTX 3080 project.

The audit never saves, never starts model servers, and restores show-control,
timeline, audio, visual-path, seed, and color state before returning.
"""
...
def audit_3080_controls():
    ...
    if control is None:
        raise RuntimeError(
            f"Missing the {PROJECT_VARIANT} show_control component."
        )
    ...
    if callbacks is None or execute_callbacks is None:
        raise RuntimeError(
            f"Missing a required {PROJECT_VARIANT} callback DAT."
        )
    ...
    try:
        ...
        check(
            "control:Visualpath:menu",
            list(control.par.Visualpath.menuNames)
            == ["original", "human_figures"]
            and list(control.par.Visualpath.menuLabels)
            == ["Original Story Visuals", "Human Figures"],
            (
                list(control.par.Visualpath.menuNames),
                list(control.par.Visualpath.menuLabels),
            ),
        )
        ...
        preserved_audio_source = str(connector.par.Audiosource.eval())
        for visual_path, parameter_name in (
            ("original", "Scenejson"),
            ("human_figures", "Humanfigurejson"),
        ):
            set_control("Visualpath", visual_path)
            expected_path = str(getattr(connector.par, parameter_name).eval())
            check(
                f"switch:Visualpath:{visual_path}",
                str(connector.par.Visualpath.eval()) == visual_path
                and controller.loaded_path == controller._resolve_path(expected_path)
                and str(connector.par.Audiosource.eval()) == preserved_audio_source,
                (
                    connector.par.Visualpath.eval(),
                    controller.loaded_path,
                    connector.par.Audiosource.eval(),
                ),
            )
    ...
    print(f"AUDIT_{PROJECT_VARIANT}_CONTROLS", report)
    if failures:
        raise RuntimeError(
            f"{PROJECT_VARIANT} control audit failed {len(failures)} checks: "
            f"{failures}"
        )
    return report


audit_3080_report = audit_3080_controls()
