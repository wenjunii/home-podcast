"""Frame-separated audit of the live 5090 Parameter Execute callbacks.
...
RESULTS_KEY = "recovered_homes_5090_live_audit_results"
SNAPSHOT_KEY = "recovered_homes_5090_live_audit_snapshot"
REPORT_KEY = "recovered_homes_5090_live_audit_report"
VALUE_CONTROLS = (
    "Play",
    "Audioenabled",
    "Audiosource",
...
def audit_5090_live_events():
    ...
    if any(
        ...
        raise RuntimeError("The 5090 show-control callback network is incomplete.")
    if not bool(callbacks.par.active.eval()):
        raise RuntimeError("The 5090 control callback DAT is inactive.")
    ...
    schedule(
        (
            ...
            "print('AUDIT_5090_LIVE_EVENTS',_report)"
        )
    )

    scheduled = {
        "project": project_name,
        "controls_scheduled": 18,
        "completion_frame_delay": next_frame,
        "saved": False,
        "model_servers_started": False,
    }
    print("AUDIT_5090_LIVE_EVENTS_SCHEDULED", scheduled)
    return scheduled


audit_5090_live_events_scheduled = audit_5090_live_events()
