"""Inspect current RTX 3080 StreamDiffusionTD output without starting it.
...
GPU_NAME_PATTERN = re.compile(r"\bRTX\s*3080\b", re.IGNORECASE)
...
def audit_3080_visuals():
    ...
    for index, target_name in enumerate(
        ...
        current_status = (
            ...
            and GPU_NAME_PATTERN.search(reported_gpu) is not None
        )
        ...
        if not current_status:
            reasons.append(
                f"backend status lacks a current output name or RTX {PROJECT_VARIANT}"
            )
    ...
    print(f"AUDIT_{PROJECT_VARIANT}_VISUALS", report)
    if not ready_targets:
        raise RuntimeError(
            f"No configured {PROJECT_VARIANT} visual target has current, non-black output. "
            f"Details: {target_reports}"
        )
    return report


audit_3080_visual_report = audit_3080_visuals()
